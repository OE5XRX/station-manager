...
def test_role_to_groups_migration_assigns_users_to_correct_groups(migrator):
    ...
    old_state = migrator.apply_initial_migration([("accounts", "0001_initial")])
    # apps.get_model returns model classes — PascalCase is conventional.
    OldUser = old_state.apps.get_model("accounts", "User")  # noqa: N806
    OldUser.objects.create_user(username="alice", password="x", role="admin", email="a@x")
    OldUser.objects.create_user(username="bob", password="x", role="operator", email="b@x")
    OldUser.objects.create_user(username="carol", password="x", role="member", email="c@x")

    new_state = migrator.apply_tested_migration([("accounts", "0002_role_to_groups")])
    NewUser = new_state.apps.get_model("accounts", "User")  # noqa: N806
    Group = new_state.apps.get_model("auth", "Group")  # noqa: N806
