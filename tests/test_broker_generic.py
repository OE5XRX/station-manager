    ...
    try:
        ...
        async def scenario():
            ...
            await b.handle_subscribe(
                {"slot": 2, "module": "beacon", "capabilities": ["temperature"], "interval_ms": 20}
            )
            await asyncio.sleep(0.1)
            await b.handle_unsubscribe(
                {"slot": 2, "module": "beacon", "capabilities": ["temperature"]}
            )
        ...
        assert any(
            r["request_id"] == "g2" and not r["ok"] and r["error"]["code"] == "out_of_range"
            for r in results
        )
