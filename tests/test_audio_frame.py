...
def test_pack_parse_roundtrip():
    ...
    data = F.pack_frame(stream_ref=7, seq=42, ts=1600, flags=F.FLAG_FEC | F.FLAG_MARKER, payload=payload)
...
def test_header_is_little_endian_per_spec():
    data = F.pack_frame(stream_ref=0x0102, seq=0x0304, ts=0x05060708, flags=0, payload=b"")
...
def test_flag_bits():
    d = F.pack_frame(stream_ref=1, seq=1, ts=1, flags=F.FLAG_DTX, payload=b"x")
    mf = F.parse_frame(d)
    assert mf.dtx is True and mf.fec is False and mf.marker is False


def test_seq_and_ts_wrap():
    d = F.pack_frame(stream_ref=1, seq=0x1_0001, ts=0x1_0000_0002, flags=0, payload=b"")
    mf = F.parse_frame(d)
    assert mf.seq == 1  # wrapped at 2^16
    assert mf.ts == 2  # wrapped at 2^32
    # stream_ref must also stay in range
    d2 = F.pack_frame(stream_ref=0x1_0005, seq=0, ts=0, flags=0, payload=b"")
    assert F.parse_frame(d2).stream_ref == 5


def test_parse_bad_magic_fails_closed():
    d = bytearray(F.pack_frame(stream_ref=1, seq=1, ts=1, flags=0, payload=b"y"))
    d[0] = 0x00
    with pytest.raises(F.FrameError):
        F.parse_frame(bytes(d))
...
def test_parse_short_buffer_fails_closed():
    with pytest.raises(F.FrameError):
        F.parse_frame(b"\xa5\x01\x00")  # < 12 bytes


def test_pack_rejects_non_bytes_payload():
    with pytest.raises(TypeError):
        F.pack_frame(stream_ref=1, seq=1, ts=1, flags=0, payload="not-bytes")


def test_empty_payload_dtx_frame_is_valid():
    # DTX comfort-noise frames can be empty Opus packets.
    d = F.pack_frame(stream_ref=3, seq=9, ts=0, flags=F.FLAG_DTX, payload=b"")
    mf = F.parse_frame(d)
